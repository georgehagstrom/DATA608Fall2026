...
- Mapping transforms from a spherical surface to a 2D plane
- Many Choices
...
- Equalize areas while retaining geography
...
library(geofacet)
library(statebins)
state_unemp |> as_tibble() |> filter(year==2008) |>
  ggplot(aes(fill=rate,                                                 state=state)) + 
...
socviz::election |> filter(st != "DC") |>  ggplot(
 ...
 geom_vline(xintercept = 0, color = "gray30") +
    geom_point(size = 2) +  scale_color_manual(
      values = party_colors) + scale_x_continuous(
